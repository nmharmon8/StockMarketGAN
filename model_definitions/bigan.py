...
class GAN():
    ...
    def __init__(self, num_features, num_historical_days, generator_input_size=20):
        ...
        with tf.variable_scope("generator"):
            ...
            W2 = tf.Variable(tf.truncated_normal([128, 512]))
            b2 = tf.Variable(tf.truncated_normal([512]))

            h2 = tf.nn.sigmoid(tf.matmul(h1, W2) + b2)


            W3 = tf.Variable(tf.truncated_normal([512, generator_output_size]))
        ...
        with tf.variable_scope("encoder"):
            e_h1_size = 256
            e_W1 = tf.Variable(tf.truncated_normal([num_features*num_historical_days, e_h1_size]))
            e_b1 = tf.Variable(tf.truncated_normal([e_h1_size]))
            e_h1 = tf.nn.relu(tf.matmul(tf.reshape(self.X, [-1, num_features*num_historical_days]), e_W1) + e_b1)
            ...
            self.encoding = tf.matmul(e_h3, e_W4) + e_b4
        ...
        self.G_solver = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(self.G_loss, var_list=theta_G + theta_E)
